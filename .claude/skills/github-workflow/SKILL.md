...
description: Doctrina operativa del plugin GitHub (mcp__neoanvil__github_github). Use when invoking any GitHub action (list_prs, create_pr, merge_pr, get_checks, cross_ref, etc.), troubleshooting auth, picking PAT scopes, or wiring GitHub ↔ Jira cross-references. Covers action selection, multi-tenant credentials, audit log shape, rate-limit awareness, and the cross_ref → jira/link_issue pattern.
---
