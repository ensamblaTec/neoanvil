...
type DynamicWasmTool struct {
	name    string
	sandbox *wasmx.Sandbox
}

func (d *DynamicWasmTool) Name() string        { return d.name }
func (d *DynamicWasmTool) Description() string { return "Forjada Dinámicamente en RAM" }
func (d *DynamicWasmTool) InputSchema() MCPToolSchema {
	return MCPToolSchema{Type: "object", Properties: map[string]any{}, Required: []string{}}
}
func (d *DynamicWasmTool) Execute(ctx context.Context, args map[string]any) (any, error) {
	_, _, err := d.sandbox.Execute(ctx, "", 1000)
	return "dynamic execution ok", err
}
...
func NewForgeTool(reg *ToolRegistry, sbx *wasmx.Sandbox) *ForgeTool {
	return &ForgeTool{registry: reg, sandbox: sbx}
}

func (t *ForgeTool) Name() string { return "neo_forge_tool" }
func (t *ForgeTool) Description() string {
	return "Auto-Forja Wasm. Recibe código fuente Go de una nueva herramienta, la compila localmente de forma volátil y la inyecta al ToolRegistry."
}
func (t *ForgeTool) InputSchema() MCPToolSchema {
	return MCPToolSchema{
		Type: "object",
		Properties: map[string]any{
			"source": map[string]any{"type": "string", "description": "Source Code en lenguaje Go para la nueva herramienta."},
		},
		Required: []string{"source"},
	}
}

func (t *ForgeTool) Execute(ctx context.Context, args map[string]any) (any, error) {
	source, ok := args["source"].(string)
	if !ok {
		return nil, fmt.Errorf("source missing")
	}

	pathGo, cleanupGo, err := astx.CreateShadowFile("forged_tool_", ".go", []byte(source))
	if err != nil {
		return nil, fmt.Errorf("failed allocating Go source in RAM: %w", err)
	}
	defer cleanupGo()

	pathWasm, cleanupWasm, err := astx.CreateShadowFile("module_", ".wasm", []byte{})
	if err != nil {
		return nil, fmt.Errorf("failed allocating Wasm binary space in RAM: %w", err)
	}
	defer cleanupWasm()
	...
	wasmBytes, errRead := os.ReadFile(pathWasm)
	if errRead != nil {
		return nil, fmt.Errorf("could not read forged wasm: %w", errRead)
	}

	toolName, errLoad := t.sandbox.LoadDynamicTool(ctx, wasmBytes)
	if errLoad != nil {
		return nil, fmt.Errorf("failed loading dynamic tool into wazero: %w", errLoad)
	}

	t.registry.Register(&DynamicWasmTool{name: toolName, sandbox: t.sandbox})

	return fmt.Sprintf("Singularidad Alcanzada: Herramienta %s forjada en caliente.", toolName), nil
}
